# Taken and modified from Coste's(tlc4418) llm_optimization repository which builds on trlx trlx/trainer/accelerate_base_trainer.py:
...
@register_trainer
class CustomAccelerateRLTrainer(BaseRLTrainer):
    """
    ...
    def generate_eval(self, input_ids, attention_mask=None, **kwargs):
        ...
        # CHANGED
        kwargs = dict(self.eval_generate_kwargs, **kwargs)
        # /CHANGED
        ...
        for i_sweep, gen_sweep_value in enumerate(gen_sweep_values):
            ...
            if self.accelerator.is_main_process:
                ...
                if self.reward_fn:
                    ...
                    # CHANGED
                    columns.append("ensemble_variance")
                    if not isinstance(vars, list):
                        vars = vars.tolist()
                    columns_data.append(vars)
                    # /CHANGED
            ...
            json_rows = [
                {
                    ...
                    "answers": [row[1]],
                    "proxy_scores": [row[2]],
                    "ensemble_variance": [row[3]],
                }
                for row in rows
            ]
            json_rows[0]["kl_list"] = self.kl_list

            # save json_rows to file
            with open(
                f"{self.config.train.output_dir}/eval/eval-{self.nth_evaluation}.json",
                "w",
                encoding="utf-8",
            ) as f:
