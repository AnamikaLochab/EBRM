# # Taken and modified from Coste's(tlc4418) llm_optimization repository which build on trlx trlx/trainer/accelerate_ppo_trainer.py:
...
@register_trainer
class CustomAcceleratePPOTrainer(CustomAccelerateRLTrainer):
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        while len(ppo_rl_elements) < num_rollouts:
            ...
            # CHANGED
            stats["policy/kl"] = mean_kl_est.item()
            # /CHANGED
        ...
        # CHANGED
        if self.accelerator.is_main_process:
            self.kl_list.append(stats["policy/kl"])
        # /CHANGED
